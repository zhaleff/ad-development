    ...
    if (voting) return
      ...
      try {
        if (vote === type) {
          ...
          if (vote) {
            await supabase.rpc(vote === 'up' ? 'decrement_likes' : 'decrement_dislikes', { row_id: id })
          }
    ...
    if (notFound) return (
      ...
      return (
        ...
        {likePercent !== null && (
          ...
          <span className="text-[10px] text-white/20 tabular-nums">{likePercent}% · {total} votos</span>
          </div>
          </div>
        )}
        </motion.div>
